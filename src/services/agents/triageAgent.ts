...
export async function runTriageAgent(intake: IntakeResult, patient: PatientProfile): Promise<TriageResult> {
  ...
  if (!validPriorities.includes(result.priority)) {
    throw new Error(`Triage Agent returned invalid priority: ${result.priority}. Clinical Engine processing error.`);
  }
  if (!validActions.includes(result.suggested_action)) {
    throw new Error(`Triage Agent returned invalid action: ${result.suggested_action}. Clinical Engine processing error.`);
  }
