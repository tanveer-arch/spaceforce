...
function getNvidiaApiKey(task: 'nlp' | 'chat'): string {
  ...
  if (!key) {
    throw new Error(`NVIDIA_${task.toUpperCase()}_API_KEY is not configured. Clinical Engine Unavailable.`);
  }
  return key;
}
...
function getElevenLabsApiKey(): string {
  ...
  if (!key) {
    throw new Error('ELEVENLABS_API_KEY is not configured. Audio Engine Unavailable.');
  }
  return key;
}
...
export async function callNvidiaChat(
  ...
  if (model === 'nvidia/nemotron-3-super-120b-a12b') {
    ...
    requestBody.reasoning_budget = 4096; // Adjust reasoning budget as needed
  }
  ...
  const response = await fetch(`${baseUrl}/chat/completions`, {
    ...
    headers: {
      ...
      // OpenRouter recommended headers (ignored by NVIDIA)
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `NVIDIA NIM API Error (${response.status}): ${errorText}. Clinical Engine Unavailable - Real-time processing failed.`
    );
  }
  ...
  if (!data.choices || data.choices.length === 0 || !data.choices[0].message?.content) {
    throw new Error('NVIDIA NIM returned empty response. Clinical Engine Unavailable - Real-time processing failed.');
  }
 ...
 * Throws on parse failure — no fallback.
 */
export async function callNvidiaChatJSON<T>(
  ...
  try {
    ...
    throw new Error(
      `NVIDIA NIM returned unparseable JSON: ${cleaned.substring(0, 200)}... Clinical Engine Unavailable - Real-time processing failed.`
    );
  }
}
...
export async function callElevenLabsSTT(audioBase64: string): Promise<string> {
  ...
  // ElevenLabs STT expects multipart/form-data with an audio file
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => 'Unknown error');
    throw new Error(
      `ElevenLabs STT Error (${response.status}): ${errorText}. Speech recognition failed.`
    );
  }

  const data = await response.json() as any;
  if (!data.text) {
    throw new Error('ElevenLabs STT returned empty transcript. Speech recognition failed.');
  }
...
/**
 * ElevenLabs Text-to-Speech.
 * Converts text to spoken audio. Returns base64-encoded MP3 audio.
 * Uses the "George" voice (JBFqnCBsd6RMkjVDRZzb) with eleven_v3 model.
 */
export async function callElevenLabsTTS(text: string): Promise<string> {
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => 'Unknown error');
    throw new Error(
      `ElevenLabs TTS Error (${response.status}): ${errorText}. Text-to-speech failed.`
    );
  }

  // Response is binary audio — convert to base64
