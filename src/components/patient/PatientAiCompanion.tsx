...
export const PatientAiCompanion: React.FC<Props> = ({ activePatient }) => {
  ...
  useEffect(() => {
    if (!histories[activePatient.id]) {
      const initialMessage: PatientCompanionMessage = {
        ...
        data: {
          simpleExplanation: `Hello ${activePatient.name}! 👋 I am your HealthSense AI Health Companion. I'm here to answer your questions about your health, lab reports, food, exercise, and medicines in simple, easy-to-understand language.`,
          keyTakeaways: [
            `Your recent HbA1c is ${activePatient.vitals.hba1c}% (this measures your average blood sugar over the last 3 months).`,
            `Your blood pressure is ${activePatient.vitals.bpSystolic}/${activePatient.vitals.bpDiastolic} mmHg.`,
            `Your doctor, ${activePatient.primaryDoctor}, has designed a care plan to keep your heart and kidneys happy and healthy!`,
          ],
          practicalTip: 'Tip of the Day: Drinking a glass of water first thing in the morning and going for a 15-minute walk after meals helps keep your energy steady!',
          encouragement: 'Every small step you take today makes a big difference for your health tomorrow! You are doing great. 😊',
          followUpQuestions: [
            'Can I eat mango?',
            'Can I eat rice?',
  ...
  const exampleQuestions = [
    ...
    { label: '📋 Explain my doctor recommendations', query: "What are my doctor's main recommendations?" },
  ...
  const generateLocalPatientResponse = (q: string, p: Patient): PatientCompanionResponseData => {
    ...
    // 8. General Health Answer
    return {
      simpleExplanation: `Thank you for asking, ${p.name}! I am always here to explain your health, diet, and lab reports in easy, everyday language. 😊`,
      keyTakeaways: [
        `Your health index shows positive progress under Dr. ${p.primaryDoctor}'s care.`,
        `Your recent blood pressure (${p.vitals.bpSystolic}/${p.vitals.bpDiastolic} mmHg) and HbA1c (${p.vitals.hba1c}%) are being monitored closely.`,
        'Simple daily habits like drinking water, eating fiber, walking, and taking medicines on time make a huge difference.',
      ],
      practicalTip: 'Try eating dinner at least 2 hours before bedtime to give your digestive system time to relax!',
      encouragement: 'Remember: health is a journey of small daily steps. Keep going, you are on the right track! 🌟',
      followUpQuestions: ['Can I eat mango?', 'Can I eat rice?', 'Can I walk every day?', 'Why do I need HbA1c?'],
    };
  };
